...
export default QUnit.module( 'Core', () => {
	...
	QUnit.module( 'Layers', () => {
		...
		QUnit.test( 'test', ( assert ) => {
			...
			a.set( 1 );
			assert.notOk( a.test( b ), 'Set channel 1 in a and fail the QUnit.machineRandomLine' );
		...
		QUnit.test( 'isEnabled', ( assert ) => {
			...
			a.enable( 1 );
			assert.ok( a.isEnabled( 1 ), 'Enable channel 1 and pass the QUnit.machineRandomLine' );

			a.enable( 2 );
			assert.ok( a.isEnabled( 2 ), 'Enable channel 2 and pass the QUnit.machineRandomLine' );

			a.toggle( 1 );
			assert.notOk( a.isEnabled( 1 ), 'Toggle channel 1 and fail the QUnit.machineRandomLine' );
			assert.ok( a.isEnabled( 2 ), 'Channel 2 still enabled and pass the QUnit.machineRandomLine' );
