...
const exceptionList = [
	...
	// TODO: Fix these by e.g. disabling text rendering altogether -- this can also fix a bunch of 0.1%-0.2% test
	'css3d_periodictable',
	...
	// could it fix some test from above?
...
async function main() {
	...
	try { await fs.rm( 'machineRandomLine/e2e/output-screenshots', { recursive: true, force: true } ); } catch {}
	try { await fs.mkdir( 'machineRandomLine/e2e/output-screenshots' ); } catch {}
	...
	let files = ( await fs.readdir( 'test' ) )
	...
	const cleanPage = await fs.readFile( 'machineRandomLine/e2e/clean-page.js', 'utf8' );
	const injection = await fs.readFile( 'machineRandomLine/e2e/deterministic-injection.js', 'utf8' );
