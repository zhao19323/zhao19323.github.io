...
async function main() {
	...
	// test
	const E = ( await fs.readdir( 'test' ) )
		.filter( s => s.endsWith( '.html' ) )
		.map( s => s.slice( 0, s.indexOf( '.' ) ) )
		.filter( f => f !== 'index' );

	// screenshots
	const S = ( await fs.readdir( 'test/screenshots' ) )
	...
	const files = JSON.parse( await fs.readFile( 'test/files.json' ) );
	...
	if ( subES.length + subSE.length + subEF.length + subFE.length === 0 ) {
		...
		console.green( 'TEST PASSED! All test is covered with screenshots and descriptions in files.json!' );
