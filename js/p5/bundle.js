...
ObjectFit.prototype.polyfill = function(options) {
    this.selector = options && options.selector !== 'undefined' ? options.selector : '.image-background:not(.breakpoint-stack) images';
...
(function(exports) {
  ...
  SrcsetInfo.prototype._parse = function() {
    // 1. Let input be the value of the images element's srcset attribute.
  ...
  function main() {
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      if (srcset) {
        ...
        // Replace the <images src> with this image.
