...
function setup() {
    ...
    //sky
    for (let y = -20; y < height; y += 3) {
        for (let x = -20; x < width + 20; x += 3) {
            strokeScale = map(y, -20, height, 3, 0.1)
            strokeWeight(strokeScale)
            point(x - noise(x / 100, y / 10) * 7, y + noise(x / 100, y / 10) * 5)
        }
    }
    sky = get()
}
...
function draw() {
    ...
    let x = width * 0.3
    beginShape()
    for (let y = height; y > height * 0.33; y -= 1) {
        let off = simplex.noise2D(y / 200 - frameCount / 200, 100)
        let amount = map(y, height, height * 0.33, width / 20, width / 3)
        let roadShrink = map(y, height, height * 0.33, 0, width / 7)
        let xOff = off * amount

        vertex(x + xOff + roadShrink, y)
        line(x + xOff + roadShrink, y, x + xOff + roadShrink, height)

    }

    for (let y = height * 0.33; y < height + height / 5; y += 1) {
        let off = simplex.noise2D(y / 200 - frameCount / 200, 100)
        let amount = map(y, height, height * 0.33, width / 20, width / 3)
        let roadShrink = map(y, height, height * 0.33, 0, width / 7)
        let xOff = off * amount
        vertex(x + xOff + width / 3 - roadShrink, y)
        let p = createVector(x + xOff + width / 6, y)
        stripePoints.push(p)
    }

    offScreen= stripePoints.length- height/10
    ...
    for (let y = height; y > height * 0.33; y -= 1) {
        ...
        let xOff = off * amount
        vertex(x + xOff + width / 3 - roadShrink, y)
        lastX = x + xOff + width / 3 - roadShrink
    }
    let firstX = x + (simplex.noise2D((height * 0.33) / 200 - frameCount / 200, 100) * width / 3) + width / 3 - width / 7
...
class stripe {
    ...
    move() {
        let speed= map(this.extent, 0, stripePoints.length, 0.5, 5)
        this.extent+=speed
    }
    ...
    show() {
        stroke(240)
        noFill()
        let length = map(this.extent, 0, stripePoints.length, 2, floor(height /15))
        let sw= map(length, 2, floor(height/15), 1, 20)
        strokeWeight(sw)
        strokeCap(SQUARE)
        beginShape()
        for (let i = floor(this.extent); i < this.extent +length; i++) {
            vertex(stripePoints[i].x, stripePoints[i].y)
        }
        endShape()
    }
    ...
    off() {
        if (this.extent > offScreen) {
            let index = stripes.indexOf(this)
